/**
 * @file    zdt_motor.c
 * @brief   智多途步进电机驱动器 UART 通信协议实现
 * @note    基于 Emm_V5 协议，通过 UART 发送 Modbus-RTU 格式指令；
 *          支持单电机命令和多电机装载（MMCL）两种模式
 */

#include "zdt_motor.h"

uint16_t MMCL_count = 0;
uint16_t MMCL_cmd[MMCL_LEN] = {0};

/**
  * @brief    通过 UART 发送电机控制命令
  * @param    buf  ：命令字节数组
  * @param    len  ：命令字节长度
  * @retval   无
  */
...
void Emm_V5_Pos_Control(uint8_t addr, uint8_t dir, uint16_t vel, uint8_t acc, uint32_t clk, bool raF, bool snF)
{
  ...
  cmd[10] =  raF;                       // 相位置/绝对标志，false为相对运动，true为绝对值运动
  cmd[11] =  snF;                       // 多机同步运行标志，false为不启用，true为启用
  cmd[12] =  0x6B;                      // 校验字节
...
void Emm_V5_Multi_Motor_Cmd(uint8_t addr)
{
  ...
  if(MMCL_count > 0)
  {
    ...
    for(i = 0, j = 4; i < MMCL_count; i++, j++) { cmd[j] = (uint8_t)MMCL_cmd[i]; }
    cmd[j] = 0x6B; ++j;                  // 校验字节
...
void Emm_V5_Origin_Modify_Params(uint8_t addr, bool svF, uint8_t o_mode, uint8_t o_dir, uint16_t o_vel, uint32_t o_tm, uint16_t sl_vel, uint16_t sl_ma, uint16_t sl_ms, bool potF)
{
  ...
  cmd[12] =  (uint8_t)(sl_vel >> 8);    // 碰撞检测转速(RPM)高8位字节
  cmd[13] =  (uint8_t)(sl_vel >> 0);    // 碰撞检测转速(RPM)低8位字节
  cmd[14] =  (uint8_t)(sl_ma >> 8);     // 碰撞检测电流(Ma)高8位字节
  cmd[15] =  (uint8_t)(sl_ma >> 0);     // 碰撞检测电流(Ma)低8位字节
  cmd[16] =  (uint8_t)(sl_ms >> 8);     // 碰撞检测时间(Ms)高8位字节
  cmd[17] =  (uint8_t)(sl_ms >> 0);     // 碰撞检测时间(Ms)低8位字节
...
void Emm_V5_Auto_Return_Sys_Params_Timed(uint8_t addr, SysParams_t s, uint16_t time_ms)
{
  ...
  switch(s)                             // 信息功能码
  {
    ...
    case S_ENCL : cmd[i] = 0x31; ++i; break;  // 读取线性化校准编码器值
    ...
    case S_OAF  : cmd[i] = 0x3C; ++i; break;  // 读取电机状态+回零状态标志位（Y42）
...
void Emm_V5_Modify_OM_mA(uint8_t addr, bool svF, uint16_t om_ma)
{
  ...
  cmd[2] =  0x33;                       // 辅助码
  cmd[3] =  svF;                        // 是否存储标志
  cmd[4] =  (uint8_t)(om_ma >> 8);      // 开环工作电流高8位
  cmd[5] =  (uint8_t)(om_ma >> 0);      // 开环工作电流低8位
  cmd[6] =  0x6B;                       // 校验字节
...
void Emm_V5_Modify_FOC_mA(uint8_t addr, bool svF, uint16_t foc_mA)
{
  ...
  cmd[2] =  0x66;                       // 辅助码
  cmd[3] =  svF;                        // 是否存储标志
  cmd[4] =  (uint8_t)(foc_mA >> 8);     // 闭环最大电流高8位
  cmd[5] =  (uint8_t)(foc_mA >> 0);     // 闭环最大电流低8位
  cmd[6] =  0x6B;                       // 校验字节
...
void Emm_V5_Modify_PID_Params(uint8_t addr, bool svF, uint32_t kp, uint32_t ki, uint32_t kd)
{
  ...
  cmd[2]  =  0xC3;                      // 辅助码
  cmd[3]  =  svF;                       // 是否存储标志
  cmd[4]  =  (uint8_t)(kp >> 24);       // kp(bit24 - bit31)
  cmd[5]  =  (uint8_t)(kp >> 16);       // kp(bit16 - bit23)
  cmd[6]  =  (uint8_t)(kp >> 8);        // kp(bit8  - bit15)
  cmd[7]  =  (uint8_t)(kp >> 0);        // kp(bit0  - bit7 )
  cmd[8]  =  (uint8_t)(ki >> 24);       // ki(bit24 - bit31)
  cmd[9]  =  (uint8_t)(ki >> 16);       // ki(bit16 - bit23)
  cmd[10] =  (uint8_t)(ki >> 8);        // ki(bit8  - bit15)
  cmd[11] =  (uint8_t)(ki >> 0);        // ki(bit0  - bit7 )
  cmd[12] =  (uint8_t)(kd >> 24);       // kd(bit24 - bit31)
  cmd[13] =  (uint8_t)(kd >> 16);       // kd(bit16 - bit23)
  cmd[14] =  (uint8_t)(kd >> 8);        // kd(bit8  - bit15)
  cmd[15] =  (uint8_t)(kd >> 0);        // kd(bit0  - bit7 )
  cmd[16] =  0x6B;                      // 校验字节
...
/**
  ...
  * @param    addr     ：电机地址
  * @param    svF      ：是否存储标志，false为不存储，true为存储
  * @param    tch      ：总通道数，默认为192
  * @param    nch      ：每个电机占用的通道数，默认为1
  * @param    mode     ：运动模式，默认为1，0为相对位置模式，1为绝对坐标式位置运动
  * @param    vel      ：单通道模式的运动速度，默认值为1000RPM
  * @param    acc      ：加速度
  * @param    vel_step ：双通道模式速度步长，默认值为10
  * @param    pos_step ：双通道模式运动步长，默认值为100
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void Emm_V5_Modify_DMX512_Params(uint8_t addr, bool svF, uint16_t tch, uint8_t nch, uint8_t mode, uint16_t vel, uint16_t acc, uint16_t vel_step, uint32_t pos_step)
{
  ...
  cmd[6]  =  nch;                       // 每个电机占用通道数
  cmd[7]  =  mode;                      // 运动模式
  cmd[8]  =  (uint8_t)(vel >> 8);       // 单通道运动速度高8位
  cmd[9]  =  (uint8_t)(vel >> 0);       // 单通道运动速度低8位
  ...
  cmd[15] =  (uint8_t)(pos_step >> 16); // 双通道运动步长(bit16 - bit23)
  cmd[16] =  (uint8_t)(pos_step >> 8);  // 双通道运动步长(bit8  - bit15)
...
void Emm_V5_Modify_Otocp(uint8_t addr, bool svF, uint16_t otp, uint16_t ocp, uint16_t time_ms)
{
  ...
  cmd[8]  =  (uint8_t)(time_ms >> 8);   // 检测时间高8位
  cmd[9]  =  (uint8_t)(time_ms >> 0);   // 检测时间低8位
  cmd[10] =  0x6B;                      // 校验字节
...
void Emm_V5_Modify_Heart_Protect(uint8_t addr, bool svF, uint32_t hp)
{
  ...
  cmd[2]  =  0x38;                      // 辅助码
  cmd[3]  =  svF;                       // 是否存储标志
  cmd[4]  =  (uint8_t)(hp >> 24);       // 心跳保护时间(bit24 - bit31)
  cmd[5]  =  (uint8_t)(hp >> 16);       // 心跳保护时间(bit16 - bit23)
  cmd[6]  =  (uint8_t)(hp >> 8);        // 心跳保护时间(bit8  - bit15)
  cmd[7]  =  (uint8_t)(hp >> 0);        // 心跳保护时间(bit0  - bit7 )
  cmd[8]  =  0x6B;                      // 校验字节
...
void Emm_V5_Modify_Integral_Limit(uint8_t addr, bool svF, uint32_t il)
{
  ...
  cmd[2]  =  0x57;                      // 辅助码
  cmd[3]  =  svF;                       // 是否存储标志
  cmd[4]  =  (uint8_t)(il >> 24);       // 积分限幅(bit24 - bit31)
  cmd[5]  =  (uint8_t)(il >> 16);       // 积分限幅(bit16 - bit23)
  cmd[6]  =  (uint8_t)(il >> 8);        // 积分限幅(bit8  - bit15)
  cmd[7]  =  (uint8_t)(il >> 0);        // 积分限幅(bit0  - bit7 )
  cmd[8]  =  0x6B;                      // 校验字节
